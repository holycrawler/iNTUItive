...
((): void => {
    "use strict";
    const competition: WorldCupLevel = (document.querySelector(
        "select[name='countrySelect'] option:checked"
    ) as HTMLOptionElement).value as WorldCupLevel;
    const season: WorldCupSeason = (document.querySelector(
        "select[name='selectSeason'] option:checked"
    ) as HTMLOptionElement).value as WorldCupSeason;
